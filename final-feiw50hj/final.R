...
name <- "Jim Hsiao (Yuan Chun)"
github.name <- "jimhsiao"
email.address <- "jimhsiao95@gmail.com"
...
y <- list()
for(i in 1:10) {
  y[[i]] <- seq(from = i, by = i, length.out = 1000)
}
...
# i.e. each element is one of the numbers 1 through 6, with equal probability.
  set.seed(31415)
v <- sample(x = (1:6), size = 100, replace = TRUE)
...
set.seed(344)
m <- matrix(rnorm(n = 100, mean = 8), ncol = 10)
...
rand <- sample((1:578), size = 200, replace = FALSE)
cw4 <- ChickWeight[c(rand), ]
...
#  
bdiff <- abs(mean(infants$bwt[infants$age<35], na.rm = T) - mean(infants$bwt[infants$age>=35], na.rm = T))
...
temp = coef(lm(mtcars$hp ~ mtcars$qsec))
...
plot(x = mtcars$wt, y = mtcars$disp, xlab = "weight in 1000lb", 
     ylab = "engine displacement in cu. in.", col = 2 + mtcars$am)
...
plot(x = WorldBank$fertility.rate, y = WorldBank$life.expectancy, pch = ".", xlab = "Fertility rate"
