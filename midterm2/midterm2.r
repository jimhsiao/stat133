...
x <- seq(from = 6, length.out = 1000, by = 6)
...
temp <- seq(1,13)
temp <- rep(temp, 4)
w <- sample(temp, replace = FALSE, 52)
...
m <- matrix(nrow = 10, ncol = 10, data = rexp(rate = 3, n = 100))
...
set.seed(71)
l <- list(rpois(100, 5), rpois(100, 5), rpois(100, 5), rpois(100, 5), rpois(100, 5), rpois(100, 5), rpois(100, 5), 
...
plot(iris$Petal.Width ~ iris$Petal.Length, xlab = "Petal Length", ylab = "Petal Width", col = c("yellow", "red", "green"))


# [3 pt]
# Make a scatterplot of ( sepal length / petal length) as a function of index (order)
# Color the plotting symbol by Species (any 3 colors)
scatter.smooth(iris$Sepal.Length/iris$Petal.Length, iris$Species, col = c("red", "blue", "yellow"))
...
firstElem <- function(x) {
  return(x[1])
}

first.cache <- unlist(lapply(Cache500,firstElem))
...
mean.cache <- unlist(lapply(Cache500, meanOf))
