...
presidents <- presidents[c(-length(presidents))]
...
speechYr <- as.numeric(sapply(tempDates, dateToYear))
#speechMo <- sapply(tempDates, dateToMonth)
speechMo <- strsplit(tempDates, " ")
speechMo <- sapply(speechMo, "[", 1)
speechYr <- speechYr[c(-length(speechYr))]
speechMo <- speechMo[c(-length(speechMo))]
...
for(i in 1:n.speeches){
  temp <- paste(speeches[(breaks[i] + 6) : (breaks[i+1] - 1)] , collapse = " ", sep = " ")
  speechesL[[i]] <- unlist(strsplit(temp, "[.?!]"))
}
...
# Output : words, a character vector where each element is one word 
  ...
  sents = gsub("'", "", sentences)
  sents = tolower(gsub("[0-9]+s*", "", sents))
    
  # Drop the words (Applause. and Laughter.)
  sents = gsub("\\(applause.\\)", "", sents)
  sents = gsub("\\(laughter.\\)", "", sents)
  

  
  # Split the text up by blanks and punctuation  (hint: strsplit, unlist)
  words = unlist(strsplit(sents, "[[:punct:][:blank:]]+"))
...
toVectors <- function(x) {
  count = table(x)
  temp = emptyVec
  temp[names(count)] = count
  return(temp)
}

wordVecs = lapply(speechWords, toVectors)
...
splitdata <- split(data.frame(t(wordMat)), speechesDF$Pres)
summed <- sapply(splitdata, colSums)
presidentWordMat <- matrix(summed, ncol = length(unique(presidents)), byrow = FALSE) 
...
plot(x = mds, type = "n", main = "Presidents", xlab = "", ylab = "")
text(x = mds, labels = speechesDF, col = cols[presParty[rownames(presDist)]])
...
avgWord = speechesDF$char/speechesDF$word

plot(speechesDF$year, avgWord, xlab = "year", ylab = "average word length")
avgSent = speechesDF$word/speechesDF$sent
plot(speechesDF$year, avgSent, xlab = "year", ylab = "average sentence length")
