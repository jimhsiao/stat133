...
temp = data.frame(wr1500m$year, wr1500m$times)
# Your ggplot / qplot command:

ggplot(temp, aes(x = wr1500m.year, y = wr1500m.times)) + geom_step(data = temp)
...
qplot(wr1500m$accurate_year, wr1500m$times)
...
temp = data.frame(wr1500m$accurate_year, wr1500m$times)
# Your ggplot command:
plotA <- ggplot(temp, aes(wr1500m.accurate_year, y = wr1500m.times)) + geom_step(data = temp)
plotA <- step_plot +geom_segment(aes(x = 1890, y = wr_1998, xend = 2014, yend = wr_1998))
...
plotA + geom_vline(xintercept = 1944, colour="green") + 
...
plotA + geom_vline(xintercept = 1944, colour="green") + 
  ...
  geom_text(aes(label=wr1500m$athlete[wr1500m$year == 1944], x=1944, y=wr_1944), colour="blue") + 
  geom_text(aes(label=wr1500m$athlete[wr1500m$year == 1998], x=1998, y=wr_1998), colour="blue") + 
  labs(title="World Record 1500m Times", x="Year", y="World Record Time (s)")
...
temp = data.frame(SO2012Ctry$pop, SO2012Ctry$GDP)
gdpPop <- ggplot(temp, aes(x = SO2012Ctry$pop, y = SO2012Ctry$GDP)) + geom_point(data = temp, size = 1) + 
  labs(title="Population vs. GDP, 2012 Olympics", x="Population", y="GDP")
...
temp = data.frame(SO2012Ctry$pop, SO2012Ctry$GDPPP)
gdpPop <- ggplot(temp, aes (x = SO2012Ctry.pop, y = SO2012Ctry.GDPPP)) + scale_x_log10() + scale_y_log10() + geom_point(data = temp, size = SO2012Ctry$Total/3) 
...
temp <- SO2012Ctry$GDPPP[order(SO2012Ctry$pop, decreasing = TRUE)]
top5 <- temp[1:5]
for (i in 1:5) {
  gdpPop <- gdpPop +
    geom_text(x = log(SO2012Ctry$pop[SO2012Ctry$GDPPP == top5[i]]), y = log(top5[i]), label = SO2012Ctry$Country[SO2012Ctry$GDPPP == top5[i]],colour="red")
}
...
mData <- map_data("world") 
worldMap <- ggplot() +  geom_polygon(data=mData, aes(x=mData$long, y=mData$lat, group=group), colour = 'grey90')
worldMap
...
with_gold <- ggplot() +  geom_polygon(data=mData, aes(x=mData$long, y=mData$lat, group=group), colour = 'grey90') + geom_point(data = wonMedal, aes(x = wonMedal$long, y = wonMedal$lat), size = wonMedal$Total/3, colour = '#FEB24CAA') +  ggtitle("Medals Won Around World") + xlab("Longitude") + ylab("Latitude") 
with_gold_map
...
athTab <- data.frame(athletes$Sex, athletes$Sport)
with(athletes, table(Sport, Sex))
barPlot <- ggplot(athletes, aes(x = Sport, fill = Sex)) + geom_bar(position="dodge", stat="bin")
barPlot
...
# Your ggplot commands
barPlot <- barPlot + labs(title="Athletes Sex by Sport") + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
barPlot
